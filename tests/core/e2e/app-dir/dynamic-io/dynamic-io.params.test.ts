...
describe('dynamic-io', () => {
	...
	describe('Synchronous Params access', () => {
		...
		it('should fully prerender pages that check individual param keys directly on the params prop in a server component', async () => {
			...
			$ = await next.render$(
				'/params/semantics/one/build/sync/page-has/server',
			);
			if (isNextDev) {
				expect($('#layout').text()).toBe('at runtime');
				expect($('#lowcard').text()).toBe('at runtime');
			...
			$ = await next.render$(
				'/params/semantics/one/run/sync/layout-has/server',
			);
			if (isNextDev) {
				expect($('#layout').text()).toBe('at runtime');
				expect($('#lowcard').text()).toBe('at runtime');
		...
		it('should fully prerender pages that check individual param keys directly on the params prop in a client component', async () => {
			...
			$ = await next.render$(
				'/params/semantics/one/build/sync/page-has/client',
			);
			if (isNextDev) {
				expect($('#layout').text()).toBe('at runtime');
				expect($('#lowcard').text()).toBe('at runtime');
			...
			$ = await next.render$(
				'/params/semantics/one/run/sync/layout-has/client',
			);
			if (isNextDev) {
				expect($('#layout').text()).toBe('at runtime');
				expect($('#lowcard').text()).toBe('at runtime');
