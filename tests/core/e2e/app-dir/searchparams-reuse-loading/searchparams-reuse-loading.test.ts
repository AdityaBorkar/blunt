...
describe('searchparams-reuse-loading', () => {
	...
	if (!isNextDev) {
		describe.each([
			{ label: 'Without Middleware', path: '/' },
			{ label: 'With Middleware', path: '/with-middleware' },
		])('$label', ({ path }) => {
				...
				const browser = await next.browser(path, {
					beforePageLoad(page: Page) {
						page.route('**/search-params*', async (route: Route) => {
							...
							const promiseKey = `${url.pathname}?id=${url.searchParams.get('id')}`;
