...
import { check, findPort } from 'next-test-utils';
...
describe('next-image-proxy', () => {
	...
	it('loads images without any errors', async () => {
		...
		const expected = JSON.stringify({ failCount: 0, fulfilledCount: 4 });
		await check(() => JSON.stringify({ failCount, fulfilledCount }), expected);
		await browser.close();
	});
