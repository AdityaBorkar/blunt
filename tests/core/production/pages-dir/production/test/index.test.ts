...
describe('Production Usage', () => {
	...
	if (process.env.BROWSER_NAME !== 'safari') {
		it.each([
			...
			{ hash: '##hello' },
			{ hash: '#hello?world' },
			{ hash: '#a', query: { hello: 'world' }, search: '?hello=world' },
			{ hash: '#a', query: { hello: '' }, search: '?hello' },
			{ hash: '#a', query: { hello: '' }, search: '?hello=' },
		])(
	...
	it('should output traces', async () => {
		...
		for (const check of checks) {
			require('node:console').log('checking', check.page);
	...
	describe('API routes', () => {
		...
		it('should work with dynamic params and search string', async () => {
			...
			expect(body).toEqual({ post: 'post-1', val: '1' });
		});
	});
