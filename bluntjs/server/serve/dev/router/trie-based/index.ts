 ...
 */
export function trieBasedRouter(_options: {
	rootDir: string;
	include?: string[];
	exclude?: string[];
}) {}
...
function matchRoute(path: string): {
	...
	for (let i = 0; i < segments.length; i++) {
		...
		if (currentNode.catchAll?.paramName) {
		...
		if (currentNode.optionalCatchAll?.paramName) {
 ...
 */
function _resolveRouteComponents(path: string): {
	page?: string;
	layouts: string[];
	loading?: string;
	error?: string;
